...
// The picker asks the grid the other way up — "what runs this namespace" rather than "what
// does this aggregate miss" — and the two have to be the same cells or the tree would call a
// namespace covered while `--lint` called it a gap.
func TestReachesIsTheGridTransposed(t *testing.T) {
	tasks, reach := codocShape()
	g := BuildGrid(tasks, reach, nil)
	got := g.Reaches()

	if !slices.Equal(got["backend"], []string{"test", "build", "lint"}) {
		t.Errorf("backend is run by all three, in Taskfile order; got %v", got["backend"])
	}
	if !slices.Equal(got["web"], []string{"build", "lint"}) {
		t.Errorf("root test never reaches web:test, so it must not claim web; got %v", got["web"])
	}
	// `site:build` is declared and never reached, which is exactly the gap `--lint` reports.
	if len(got["site"]) != 0 {
		t.Errorf("site is a gap, not a coverage; got %v", got["site"])
	}
	// `wt` answers no aggregate's verb at all.
	if len(got["wt"]) != 0 {
		t.Errorf("wt answers nothing; got %v", got["wt"])
	}
}
