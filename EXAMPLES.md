...
Passing tasks stay collapsed with a line count. Every task carries its own clock, ticking
while it runs — during a slow build the useful question is which *step* is slow.

`esc` leaves without stopping the run; the picker header then says `▶ ci running · v to
watch`. `v` goes back. `x` stops it.
